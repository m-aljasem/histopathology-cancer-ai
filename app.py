"""Streamlit app for histopathology classification"""
...
uploaded_file = st.file_uploader("Upload histopathology image", type=['png', 'jpg'])
if uploaded_file:
    img = Image.open(uploaded_file).resize((50, 50))
    st.image(img, caption="Histopathology Image")
    ...
    if st.button("Classify"):
        ...
        result = "IDC (+) Cancer" if np.argmax(pred[0]) == 1 else "No Cancer"
        ...
        if st.button("Explain Prediction (SHAP)"):
            try:
                ...
                shap_values = explainer.explain_instance(img_array, plot=False, class_idx=np.argmax(pred[0]))
